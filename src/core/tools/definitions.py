...
def define_document_tools():
    ...
    # Document: Store Artifact
    registry.register(
        Tool(
            schema=create_tool_schema(
                name="store_artifact",
                description="Persist a generated file into the durable artifact store so the user "
                "can view, share, and manage it from the Artifacts tab. "
                "Pass 'source_path' = the filepath returned by create_html/create_pdf/create_docx "
                "(or a file written by python_execute). Files generated by those tools live in "
                "ephemeral temp storage that is purged nightly; storing an artifact copies it to "
                "permanent storage. "
                "Set 'make_public' to true to get a permanent shareable link immediately; "
                "otherwise the artifact stays private and the user can create a 5-minute "
                "temporary link from the Artifacts tab. "
                "Use this after generating an HTML page/report the user wants to keep or share.",
                parameters_model=StoreArtifactRequest,
            ),
            executor=None,
            service_name="system",
            requires_auth=False,
